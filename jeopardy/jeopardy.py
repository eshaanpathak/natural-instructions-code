...
def read_file(filename):
    ...
    random.shuffle(rounds["Jeopardy!"])
    random.shuffle(rounds["Double Jeopardy!"])
    random.shuffle(rounds["Final Jeopardy!"])
...
def get_instances(round_qa):
    ...
    for category, question, answer in round_qa:
        for example in examples:
            if example.lower() in question.lower() or example.lower() == question.lower():
                continue
        question = re.sub("^'|'$", "", question)
        instance_input = "Category: " + category + " \n" + "Clue: " + question
        if instance_input.lower() in instance_set:
            continue
        instance = {"input" : instance_input, "output" : [answer]}
        instances.append(instance)
        instance_set.add(instance_input.lower())
    print(len(instances))
